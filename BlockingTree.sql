...
--    PURPOSE:
--
--        This stored procedure is to display a tree of all sessions that are locking, visualizing the lock heirarchy.
--      
--    TARGET: 
--        SQL Server
--
...
CASE WHEN (LEN(LEVEL)/4 - 1) = 0 THEN 'HEAD -  '
	ELSE '|------  ' END + CAST (SPID AS NVARCHAR (10)) + N' ' + BATCH AS BLOCKING_TREE 
FROM BLOCKERS 
ORDER BY LEVEL ASC
GO
